...
export function AdminNewBusinessUnitButton({ className, dataInstance, isTreeGrid = false }: { className?: string, dataInstance: string, isTreeGrid?: boolean }) {
    ...
    return (<button className={clsx("bg-primary-400 text-white w-48 h-10 rounded-md hover:bg-primary-600", className)}
        onClick={handleNewBu}>Create New Bu and Link</button>)
