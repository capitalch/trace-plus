...
export function generateSalesInvoicePDF(
  ...
  const { tranH, billTo, salePurchaseDetails, tranD, extGstTranD, shippingInfo, businessContacts } = invoiceData;
  const companyInfo: UnitInfoType = Utils.getUnitInfo() || {};

  // Helper function to transform businessContacts to ContactsType structure for institution sales
  // Institution sales have billTo as null, but businessContacts contains full customer information
  function transformBusinessContactsToContactType(data: any): any {
  ...
  // Determine effective billTo for PDF display
  // For institution sales, billTo will be null, so use businessContacts
  const effectiveBillTo = billTo || transformBusinessContactsToContactType(businessContacts);
  ...
  const rightColumnWidth = pageWidth * 0.3 - marginLeft; // 30% width for right section
  const maxLeftColumnWidth = leftColumnWidth - 25 - 10; // Reduce by 10px for horizontal gap
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text(effectiveBillTo?.contactName || '', leftColumnX, leftY);
  leftY += 10;

  doc.setFont('helvetica', 'normal');
  if (effectiveBillTo?.gstin) {
    doc.text(`GSTIN: ${effectiveBillTo.gstin}`, leftColumnX, leftY);
    leftY += 10;
  }

  const billToAddress = [effectiveBillTo?.address1,
  effectiveBillTo?.address2,
  effectiveBillTo?.pin ? `Pin: ${effectiveBillTo?.pin}` : '',
  effectiveBillTo?.city,
  effectiveBillTo?.state,
  effectiveBillTo?.country,
  effectiveBillTo?.mobileNumber ? `Ph: ${effectiveBillTo?.mobileNumber}` : '',
  effectiveBillTo?.otherMobileNumber ? `, ${effectiveBillTo?.otherMobileNumber}` : '',
  effectiveBillTo?.email ? `Email: ${effectiveBillTo?.email}` : ''
  ]
  ...
  // Use shipTo if available, otherwise fall back to effectiveBillTo (Customer Details)
