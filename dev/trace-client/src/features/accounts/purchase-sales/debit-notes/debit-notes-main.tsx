...
export function DebitNotesMain() {
  ...
  const inputClass = "border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-300 text-right font-medium text-sm w-full h-10 rounded-lg px-3 transition-all duration-200";
  const isIgst = watch('igst');

  return (
    <div className="bg-gray-50 p-8 rounded-xl shadow-sm max-w-7xl mx-auto">
      {/* Header Section */}
      <div className="mb-8 relative">
        ...
        <div className="grid grid-cols-12 gap-6 bg-white p-6 rounded-lg shadow-sm">
          {/* Auto ref no */}
          <FormField label="Auto Ref No" className="col-span-3">
            <input
              type="text"
              className={clsx("bg-gray-100 rounded-lg h-10 px-3 text-sm", inputClass)}
              readOnly
              disabled
          ...
          {/* Tran date */}
          <FormField label="Date" required error={errors?.tranDate?.message} className="col-span-3">
            <input
              type="date"
              className={clsx(inputClass, errors?.tranDate && errorClass)}
              {...register("tranDate", {
          ...
          {/* User ref no */}
          <FormField label="User Ref No" className="col-span-3">
            <input
              type="text"
          ...
          {/* Remarks */}
          <FormField className="col-span-3" label="Remarks">
            <textarea
              rows={3}
      ...
      <div className="grid grid-cols-12 gap-6 bg-white p-6 rounded-lg shadow-sm">
        {/* Amount */}
        <FormField label="Amount" required error={errors?.amount?.message} className="col-span-3">
          <ControlledNumericInput
            className={clsx(inputClass, errors?.amount && errorClass)}
            fieldName="amount"
            required={true}
        ...
        {/* GST Applicable */}
        <FormField label="GST Applicable?" className="col-span-3 flex flex-col items-center">
      ...
      {/* Line Items Section */}
      <div className="grid grid-cols-12 gap-6 bg-white p-6 rounded-lg shadow-sm mt-6">
        {/* Debit Account */}
        <FormField
        ...
        {/* Line ref no */}
        <FormField label="Line Ref No" className="col-span-4">
          <input
            type="text"
            className={clsx(inputClass)}
        ...
        {/* Remarks */}
        <FormField className="col-span-4" label="Line Remarks">
          <textarea
            rows={3}
            className={clsx(inputClass, "resize-none")}
            placeholder="Enter remarks"
        ...
        <div className="col-span-3 flex flex-col gap-4">
          <div>
            <label className="text-gray-600 text-sm font-medium">CGST {!isIgst && <WidgetAstrix />}</label>
            <ControlledNumericInput
              className={clsx(inputClass, errors?.cgst && errorClass)}
              fieldName="cgst"
              onValueChange={(floatValue) => {
          ...
          <div>
            <label className="text-gray-600 text-sm font-medium">SGST {!isIgst && <WidgetAstrix />}</label>
            <ControlledNumericInput
              className={clsx(inputClass, errors?.sgst && errorClass)}
              fieldName="totalSgst"
              onValueChange={(floatValue) => {
          ...
          <div>
            <label className="text-gray-600 text-sm font-medium">IGST {isIgst && <WidgetAstrix />}</label>
            <ControlledNumericInput
              className={clsx(inputClass, errors?.igst && errorClass)}
              fieldName="totalIgst"
              onValueChange={(floatValue) => {
    ...
    if (activeTabIndex === 0) {
      ...
      if (id) {
        Ret = <TooltipComponent content='Print Preview' className="flex">
          <button type='button' onClick={() => handleOnPreview()} className="p-2 rounded-full hover:bg-gray-100 transition-all duration-200">
            <IconPreview1 className="text-blue-500 h-6 w-6" />
          </button>
        </TooltipComponent>
      }
      return (Ret)
