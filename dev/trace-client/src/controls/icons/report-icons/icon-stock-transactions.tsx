import { SVGProps } from "react";

export function IconStockTransactions(props: SVGProps<SVGSVGElement>) {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 48 48"
        width="1em"
        height="1em"
        {...props}
      >
        <g fill="none" strokeLinejoin="round" strokeWidth="4">
          <path
            fill="#2F88FF"
            stroke="#000"
            d="M5 7C5 5.34315 6.34315 4 8 4H32C33.6569 4 35 5.34315 35 7V44H8C6.34315 44 5 42.6569 5 41V7Z"
          ></path>
          <path
            stroke="#000"
            d="M35 24C35 22.8954 35.8954 22 37 22H41C42.1046 22 43 22.8954 43 24V41C43 42.6569 41.6569 44 40 44H35V24Z"
          ></path>
          <path stroke="#fff" strokeLinecap="round" d="M11 12H19"></path>
          <path stroke="#fff" strokeLinecap="round" d="M11 19H23"></path>
        </g>
      </svg>
    )
  }
  