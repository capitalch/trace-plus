...
class SqlAccounts:
    ...
    get_business_health = """
            --WITH "branchId" AS (VALUES (1)), "finYearId" AS (VALUES (2025)),
            WITH "branchId" AS (VALUES (%(branchId)s::INT)), "finYearId" AS (VALUES (%(finYearId)s::INT)),

            -- ========== ALL TRANSACTIONS ==========
            -- Combines sales, purchases, and stock journals
            transactions AS (
                ...
                SELECT "productId", "tranTypeId", "qty", "price", "tranDate", "dc"
                FROM "TranH" h
                JOIN "StockJournal" s ON h."id" = s."tranHeaderId"
                WHERE "branchId" = (TABLE "branchId") 
                AND "finYearId" = (TABLE "finYearId")
            ...
            -- Pivot transaction types into columns (4=Sale, 5=Purchase, 9/10=Returns, 11=Journal)
            tran_summary AS (
                SELECT 
                    "productId",
                    SUM(CASE WHEN "tranTypeId" = 4 THEN "qty" ELSE 0 END) AS "sale",
            ...
            -- ========== COMBINE OPENING + TRANSACTIONS ==========
            stock_movement AS (
                SELECT 
                    COALESCE(o."productId", t."productId") AS "productId",
                    COALESCE(o."qty", 0) AS "op",
                    ...
                    COALESCE(t."lastPurchaseDate", o."lastPurchaseDate") AS "lastPurchaseDate",
                    "openingPrice"
                FROM opening_bal o
                FULL JOIN tran_summary t ON o."productId" = t."productId"
            ),

            -- ========== LAST PRICES ==========
            last_purchase_price AS (
                SELECT DISTINCT ON("productId") "productId", "price"
                FROM transactions
                WHERE "tranTypeId" = 5
                ORDER BY "productId", "tranDate" DESC
            ),
            
            last_journal_price AS (
                SELECT DISTINCT ON("productId") "productId", "price"
                FROM transactions
                WHERE "tranTypeId" = 11
                ORDER BY "productId", "tranDate" DESC
            ...
            -- ========== CLOSING STOCK ==========
            -- Formula: Opening + Purchase - PurchaseRet - Sale + SaleRet + JournalDebits - JournalCredits
            closing_stock AS (
                SELECT 
                    COALESCE(sm."productId", lp."productId") AS "productId",
                    CASE
                        WHEN COALESCE(lp."price", 0) > 0 THEN lp."price"
                        WHEN COALESCE("openingPrice", 0) > 0 THEN "openingPrice"
                        WHEN COALESCE(lj."price", 0) > 0 THEN lj."price"
                        ELSE p."purPrice"
                    END AS "lastPurchasePrice",
                ...
                FULL JOIN last_purchase_price lp ON sm."productId" = lp."productId"
                FULL JOIN last_journal_price lj ON sm."productId" = lj."productId"
                JOIN "ProductM" p ON p.id = sm."productId"
            ),
            ...
            stock_value AS (
                ...
                JOIN "ProductM" p ON p."id" = cs."productId"
            ),
            ...
            -- ========== P&L FROM BALANCE SHEET ==========
            -- Profit/Loss calculated from Assets and Liabilities only
            profit_loss AS (
                SELECT 
            ...
            -- ========== TRIAL BALANCE ==========
            -- Hierarchical account balances using recursive CTE
            trial_balance AS (
                WITH RECURSIVE account_tree AS (
                    -- Base: Leaf accounts with transactions and opening balances
                    SELECT * FROM account_data
                    
                    UNION ALL
                    
                    -- Recursive: Roll up to parent accounts
                    SELECT 
                        a."id", a."accName", a."accType", a."parentId", a."accLeaf", a."isPrimary",
                        c."opening", c."sign", c."opening_dc", c."debit", c."credit"
                    FROM account_tree c
                ...
                account_data AS (
                    -- Transactions
                    SELECT 
                        a."id", "accName", "accType", "parentId", "accLeaf", a."isPrimary",
                        0.00 AS "opening", 1 AS "sign", '' AS "opening_dc",
                    ...
                    SELECT 
                        ...
                        "dc" AS "opening_dc", 0 AS "debit", 0 AS "credit"
                    FROM "AccM" a
                ...
                account_totals AS (
                    SELECT 
                        "id", "accName", "accType", "parentId", "accLeaf", "isPrimary",
                ...
                -- Calculate closing for primary accounts only
                SELECT 
                    "id", "parentId", "accName",
            ...
            -- ========== FINAL JSON OUTPUT ==========
            SELECT json_build_object(
                'stockDiff', (SELECT row_to_json(a) FROM stock_diff a),
                'profitLoss', (SELECT "profitLoss" FROM profit_loss b),
                'openingClosingStock', (SELECT row_to_json(c) FROM stock_value c),
                'trialBalance', (SELECT json_agg(row_to_json(d)) FROM trial_balance d)
            ) AS "jsonResult"
    """
    ...
    mobile_get_products_info = """
            ...
            -- ========== ALL TRANSACTIONS ==========
            -- Sales, purchases, and stock journals
            transactions AS (
                SELECT 
                    s."productId",
                    h."tranTypeId",
            ...
            -- ========== TRANSACTION SUMMARY ==========
            -- Pivot transaction types: 4=Sale, 5=Purchase, 9/10=Returns, 11=Journal
            tran_summary AS (
                SELECT
                    "productId",
                    SUM(CASE WHEN "tranTypeId" = 4 THEN "qty" ELSE 0 END) AS "sale",
            ...
            stock_movement AS (
                SELECT
                    ...
                    COALESCE(t."purchase", 0) AS "purchase",
                    COALESCE(t."saleRet", 0) AS "saleRet",
                    COALESCE(t."purchaseRet", 0) AS "purchaseRet",
                    COALESCE(t."stockJournalDebits", 0) AS "stockJournalDebits",
                    COALESCE(t."stockJournalCredits", 0) AS "stockJournalCredits",
                    COALESCE(t."lastPurchaseDate", o."lastPurchaseDate") AS "lastPurchaseDate",
                    o."openingPrice",
                    t."lastSaleDate"
                FROM opening_bal o
            ...
            -- ========== CLOSING STOCK ==========
            -- Formula: Op + Purchase - PurchaseRet - Sale + SaleRet + JournalDebits - JournalCredits
            closing_stock AS (
                SELECT
                    COALESCE(sm."productId", lp."productId") AS "productId",
                    CASE
                        WHEN COALESCE(lp."price", 0) > 0 THEN lp."price"
                        WHEN COALESCE(sm."openingPrice", 0) > 0 THEN sm."openingPrice"
                        WHEN COALESCE(lj."price", 0) > 0 THEN lj."price"
                        ELSE p."purPrice"
                    END AS "lastPurchasePrice",
                    sm."lastPurchaseDate",
            ...
            product_report AS (
                ...
                FROM closing_stock cs
                RIGHT JOIN "ProductM" p ON p."id" = cs."productId"
                JOIN "CategoryM" c ON c."id" = p."catId"
                JOIN "BrandM" b ON b."id" = p."brandId"
                WHERE p."isActive"
                ORDER BY b."brandName", c."catName", p."label", p."info"
            )
        ...
        WITH "branchId" AS (VALUES (%(branchId)s::INT)),  "finYearId" AS (VALUES (%(finYearId)s::INT)), "tagId" AS (VALUES (%(tagId)s::INT)), "startDate" AS (VALUES (%(startDate)s::DATE)), "endDate" AS (VALUES (%(endDate)s::DATE)),
            ...
            category_hierarchy AS (
                WITH RECURSIVE rec AS (
                    ...
                    FROM "CategoryM" c 
                    INNER JOIN rec ON rec.id = c."parentId"
                )
                SELECT * FROM rec WHERE "isLeaf"
            ),

            -- Base transaction data: Sales, Purchases, and Returns (tranTypeId: 4, 5, 9, 10)
            base_transactions AS (
                SELECT 
                    h.id,
                    h.remarks AS "commonRemarks", 
                    CONCAT_WS(', ', d.remarks, s."jData"->'remarks', s."jData"->'serialNumbers') AS "lineRemarks",
                    h."tranDate", 
                    s."productId", 
                    h."tranTypeId", 
                    s.qty, 
                    ...
                    h."autoRefNo", 
                    h."timestamp",
                    CONCAT_WS(' ', c."contactName", c."mobileNumber", c."address1", c."address2") AS contact,
                    '' AS dc
                FROM "TranH" h 
                INNER JOIN "TranD" d ON h.id = d."tranHeaderId" 
                INNER JOIN "AccM" a ON a.id = d."accId" 
                INNER JOIN "SalePurchaseDetails" s ON d.id = s."tranDetailsId" 
                LEFT JOIN "Contacts" c ON c.id = h."contactsId"
                WHERE h."branchId" = (TABLE "branchId") 
                    AND h."finYearId" = (TABLE "finYearId") 
                    AND h."tranDate" <= (TABLE "endDate") 
                    AND h."tranTypeId" IN (4, 5, 9, 10)
                
                UNION ALL
                
                -- Stock Journal transactions (tranTypeId: 11)
                SELECT 
                    h.id,
                    h.remarks AS "commonRemarks", 
                    s."lineRemarks",
                    ...
                    s.dc
                FROM "TranH" h 
                INNER JOIN "StockJournal" s ON h.id = s."tranHeaderId"
                WHERE h."branchId" = (TABLE "branchId") 
                    AND h."finYearId" = (TABLE "finYearId")
            ),

            -- Account names aggregation (excluding 'sale' class for tranTypeId 4, 5)
            account_names AS (
                SELECT 
                    h.id, 
                    STRING_AGG(a."accName", ', ') AS accounts
                ...
                INNER JOIN "TranD" d ON h.id = d."tranHeaderId" 
                INNER JOIN "AccM" a ON a.id = d."accId" 
                INNER JOIN "AccClassM" c ON c.id = a."classId"
                WHERE h."tranTypeId" IN (4, 5) 
                    AND c."accClass" <> 'sale' 
                    AND h."branchId" = (TABLE "branchId") 
                    AND h."finYearId" = (TABLE "finYearId")
                GROUP BY h.id
            ...
            -- Last purchase price and date computation (for sales only)
            sales_with_purchase_info AS (
                SELECT 
                    bt.*,
                    an.accounts,
                    -- Last purchase price
                ...
                LEFT JOIN account_names an ON bt.id = an.id
                WHERE bt."tranTypeId" IN (4, 9)
            ),

            -- Fill missing purchase info from opening balances
            sales_with_complete_info AS (
                SELECT 
                    s."tranDate", 
                    s."productId", 
                    s.qty, 
                    ...
                    s."gstRate", 
                    s."tranTypeId",
                    s."salePurchaseDetailsId", 
                    s."autoRefNo"
                FROM sales_with_purchase_info s
                LEFT JOIN product_opening_balances pob ON s."productId" = pob."productId"
            ),
            ...
            sales_with_profit AS (
                SELECT 
                    *,
                    qty * (price - "lastPurchasePrice") AS "grossProfit"
                FROM sales_with_complete_info
            ),

            -- Adjust signs for sales returns (negate returns)
            normalized_sales AS (
                SELECT 
                    "tranDate", 
                    "productId", 
            ...
            -- Stock movement aggregation by product and transaction type
            stock_movements_by_type AS (
                SELECT 
                    "productId",
                    "tranTypeId",
                    SUM(CASE WHEN "tranTypeId" = 4 THEN qty ELSE 0 END) AS sale,
            ...
            -- Consolidate stock movements per product
            stock_movements_consolidated AS (
                SELECT 
                    "productId",
                    SUM(sale) AS sale,
            ...
            stock_positions AS (
                SELECT 
                    sm."productId",
                    COALESCE(pob.qty, 0) AS op,
                    sm.sale,
                    sm.purchase,
                    sm."saleRet",
                    sm."purchaseRet",
                    sm."stockJournalDebits",
                    sm."stockJournalCredits",
                    (
                        COALESCE(pob.qty, 0) + 
                        sm.purchase - 
                        sm.sale - 
                ...
                FROM stock_movements_consolidated sm
                LEFT JOIN product_opening_balances pob ON pob."productId" = sm."productId"
            )
            ...
            INNER JOIN "ProductM" p ON p.id = ns."productId"
            INNER JOIN category_hierarchy ch ON ch.id = p."catId"
            INNER JOIN "BrandM" b ON b.id = p."brandId"
            INNER JOIN stock_positions sp ON ns."productId" = sp."productId"
            WHERE ns."tranDate" BETWEEN (TABLE "startDate") AND (TABLE "endDate")
            ORDER BY ns."tranDate", ns."salePurchaseDetailsId"
    """